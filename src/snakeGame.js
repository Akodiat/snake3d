...
class SnakeGame {
    constructor(canvas, foodCount, obstacleCount, boundingBox) {
        this.canvas = canvas;
        this.boundingBox = boundingBox;

        // Create models for snake, food, and obstacles
        this.setupModel(foodCount, obstacleCount);

        // Create view, visualising the models
        this.setupView();

        // Create input listeners to control model
        this.setupInput();

        // Handle new steps
        this.snake.addEventListener("step", ()=>this.onStep());
    }

    setupModel(foodCount, obstacleCount) {
        this.snake = new Snake(this.boundingBox);
        this.food = new Food(foodCount, this.boundingBox);
        this.obstacles = new Obstacles(obstacleCount, this.boundingBox);
    }

    setupView() {
        this.view = new View(this.canvas, this.snake, this.food, this.obstacles);
    }
    ...
    onStep(onFoodCallback) {
        // Check for self collision
        const ps = this.snake.positions;
        for (let i=1; i<ps.length; i++) {
            if (collides(ps[0], ps[i])) {
                this.handleGameOver(`Game over! You collided with yourself. You got ${this.snake.length} points!`, this.snake);
                break;
            }
        }
        ...
        for (let i=0; i<os.length; i++) {
            if (collides(ps[0], os[i])) {
                this.handleGameOver(`Game over! You collided with an obstacle. You got ${this.snake.length} points!`, this.snake);
                break;
            }
        }
        ...
        for (let i=0; i<fs.length; i++) {
            if (collides(ps[0], fs[i])) {
                ...
                const newPos = this.getEmptyPos();
                fs[i] = newPos;
                onFoodCallback(ps[0], newPos);
                break;
            }
        }
    }
